...
void tempareture_process(void)
{
    ...
    //Tempareture_PID_loop(&tempareture_type);
...
void Tempareture_System_Init(tempareture_type_t* tempareture_type_init)
{
    ...
    for(int i=0;i<4;i++)
    {
        ...
        for(int i=0;i<8;i++)
        {
            Uart1_send(tempareture_type_init->contrl_package[i]);
            //printf("%x ",tempareture_type_init->contrl_package[i]);
        }
...
void Tempareture_SHT10_Get(tempareture_type_t* tempareture_type_get)
{
    for(int i=0;i<8;i++)
    {
        Uart1_send(tx_data[i]);
    }
    printf("%d,%d\n",tempareture_type_get->temp_hum_data->humidity_get,tempareture_type_get->temp_hum_data->temperature_get);
    delay_ms(20);
}
...
void Tempareture_PID_loop(tempareture_type_t* tempareture_type_loop)
{
    ...
    for(int i=0;i<8;i++)
    {
       Uart1_send(tempareture_type_loop->contrl_package[i]);
    }
    delay_ms(20);
