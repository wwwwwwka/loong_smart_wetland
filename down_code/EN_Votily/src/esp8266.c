...
void esp8266_init(void)
{
    ...
    while(esp8266_send_cmd(&Circular_queue_send,"AT+MQTTUSERCFG=0,1,\"NULL\",\"""1c102new&k1dl3eJ2RBB""\",\"""ea7040c6138d1f41524eb23f27e5392445c0dc1ccad2d2e815911ecb5d44210d""\",0,0,\"\"","OK",150));
    delay_ms(100);

    printf("AT+MQTTCLIENTID=0,k1dl3eJ2RBB.1c102s|securemode=2,signmethod=hmacsha256,timestamp=1719636740199|\r\n");
    esp8266_send_cmd(&Circular_queue_send,"AT+MQTTCLIENTID=0,\"k1dl3eJ2RBB.1c102new|securemode=2\\,signmethod=hmacsha256\\,timestamp=1723199714249|\"","OK",300);
    delay_ms(100);
    ...
    esp8266_send_cmd(&Circular_queue_send,"AT+MQTTSUB=0,\"/k1dl3eJ2RBB/1c102t/user/get\",1","OK",300);
    delay_ms(100);
