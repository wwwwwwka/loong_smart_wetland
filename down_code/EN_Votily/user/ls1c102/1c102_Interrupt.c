...
bool flag_500ms=0;
uint8_t count_1000ms=0;
bool flag_1000ms=0;

void intc_handler(void)
{
    static uint8_t receive_data_uart1=0;
    static uint8_t receive_data_uart0=0;
    ...
    if(IntReg  & IRQ_TIMER)//Timer
    {
        if(TIM_GetITStatus(TIM_FLAG_Trigger))
        {
            ...
            if(count_10ms==1) {flag_10ms=1; count_10ms=0;}
            if(count_20ms==2) {flag_20ms=1; count_20ms=0;}
            if(count_50ms==5) {flag_50ms=1; count_50ms=0;}
            if(count_100ms==10) {flag_100ms=1; count_100ms=0;}
            if(count_500ms==50) {flag_500ms=1; count_500ms=0;}
            if(count_1000ms==100) {flag_1000ms=1; count_1000ms=0;}
    ...
    {
        receive_data_uart1=UART_ReceiveData(UART1);//接收数据
        //printf("%d\n",receive_data_uart1);
        Queue_Wirte(&Circular_queue_asrpro, (uint8_t *)&receive_data_uart1, 1);
        INT_CLR = UART1_INT_CLR;
    }
    if (IntReg & UART0_INT_OUT) //Uart0
    {
        receive_data_uart0=UART_ReceiveData(UART0);
        //Queue_Wirte(&Circular_queue_asrpro, (uint8_t *)&receive_data_uart0, 1);//向队列中写入数据
        Queue_Wirte(&Circular_queue_send, (uint8_t *)&receive_data_uart0, 1);
